...
char *argv[];
{
    ...
    if (debug)
        fprintf(stderr, "finished reading command line parameters\n");
    ...
    if (debug)
        fprintf(stderr, "exiting parse_cmd_line\n");
}
...
void compute_parameters(int ttl_index)
{
    ...
    if (debug)
        fprintf(stderr, "compute pars,ttl_index=%d\n", ttl_index);
    ...
    {
        if (debug)
            fprintf(stderr, "Changing interchirp time\n");
        ...
        if ((int)(1 + (write_interval / (inter_chirp_time))) > chirps_per_write && created_arrays)
        {
            ...
            if (debug)
                fprintf(stderr, "comp_pars: chirps_per_write=%d, pkts_per_write=%d\n", chirps_per_write, pkts_per_write);
